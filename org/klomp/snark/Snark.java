...
public class Snark
{
	...
	//code for selecting port greater than 10000
	for (port = 10001; serversocket == null && port <= 10010; port++) {
                try {
                    serversocket = new ServerSocket(port);
                } catch (IOException ioe) {
                    lastException = ioe;
	...
	//converting PeerCoordinator class to a singleton design pattern
	coordinator = PeerCoordinator.createInstance(id, meta, storage, clistener);
