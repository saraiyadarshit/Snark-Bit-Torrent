...
public class PeerCoordinator implements PeerListener
{
    ...
    final static long CHECK_PERIOD = 10 * 1000; // changed from 20 to 10 seconds according to bittyrant paper

    final static int MAX_CONNECTIONS = 10;//reduced from 24 to 10

    final static int MAX_UPLOADERS = 3;//reduced from 4 to 3
    ...
    // List of uploaders
   // private List<Peer> UploadersList = new ArrayList<Peer>();
    ...
    private static PeerCoordinator myVar = null;

    private PeerCoordinator (byte[] id, MetaInfo metainfo, Storage storage,
	...
	//Converting PeerCoordinator class into singleton design pattern it will help to create only
	//one instance of a peer
     public static PeerCoordinator createInstance(byte[] id, MetaInfo metainfo, Storage storage,
        CoordinatorListener listener){

        if(myVar == null){

           myVar = new PeerCoordinator(id, metainfo, storage, listener);
    ...
    //Create a list of Uploaders
   // private void UpdateListUploaders(Peer peer){

    //UploadersList.add(peer);
...
*/
 ...
 // (Optimistically) unchoke. Should be called with peers synchronized
    void unchokePeer ()
    {
        ...
        // At the start are the peers that have us unchoked at the end the
        // other peer that are interested, but are choking us.
        List<Peer> maybeinterested = new LinkedList<Peer>();
	List<Peer> interested = new LinkedList<Peer>();
	List<Float> ratio = new ArrayList<Float>();
	...
	//Implementing the ranking algorithm according to the bittyrant paper
	//Taking download to upload ratio of data from each peer and sorting it into interested list
	//Hence the best peers are always chosen in each round
        Iterator it = peers.iterator();
        while (it.hasNext()) {
            Peer peer = (Peer)it.next();
            if (uploaders < MAX_UPLOADERS && peer.isChoking()
                && peer.isInterested()) {
                if (!peer.isChoked()) {
		maybeinterested.add(peer);
		d = peer.getDownloaded();
		u = peer.getUploaded();
		if (u != 0){
		r = d/u;
		}
		...
		while (i <= MAX_UPLOADERS ){
			x = ratio.indexOf(sorted.get(i));
			interested.add(maybeinterested.get(x));
			i++;			
			}
                }
            }
            ...
            try {
                if (storage.putPiece(piece, bs)) {
                    ...
                    log.log(Level.INFO, "Got BAD piece " + piece + " from "
                        + peer);
		//peer gave bad piece so we wont add it to interesed list by choking it
